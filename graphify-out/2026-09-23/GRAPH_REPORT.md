...
Cohesion: 0.12
Nodes (16): 💾 Backup & Data Portability (CSV), Building & Running, 👥 Customer Tabs & Settlement (Debtor Tracking), 🗄️ Database Architecture, 🤍 Dedication, 📦 Dynamic Stock & Inventory Control, ✨ Features, 🚀 Getting Started (+8 more)

## Knowledge Gaps
- **20 isolated node(s):** `HOME`, `DASHBOARD`, `ARCHIVES`, `STOCKS`, `TODAY` (+15 more)
...
- **Why does `Order` connect `Order` to `Ingredient`, `OrderViewModel`, `SeallDatabase`, `OrderRepository`, `ArchivesTab.kt`, `MainScreen.kt`, `DashboardTab.kt`, `StockItem`, `TestThemeScreen.kt`, `CsvHelper`?**
...
- **Why does `StockItem` connect `StockItem` to `Ingredient`, `OrderViewModel`, `SeallDatabase`, `OrderRepository`, `MainScreen.kt`, `CsvHelper`?**
